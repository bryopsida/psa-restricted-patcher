...
export class Admission implements IAdmission {
  ...
  async admit (pod: V1Pod): Promise<string> {
    ...
    if (!spec.securityContext.fsGroup) spec.securityContext.fsGroup = this.defaultFsGroup ?? 1001
    ...
    spec.containers = spec.containers.map((c) => {
      ...
      if (!c.securityContext.runAsGroup) c.securityContext.runAsGroup = this.defaultGid ?? 1001
      if (!c.securityContext.runAsUser) c.securityContext.runAsUser = this.defaultUid ?? 1001
      return c
    })
