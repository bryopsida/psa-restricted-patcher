...
export class Filter implements IFilter {
  ...
  private checkAnnotationMap(
    pod: V1Pod,
    matchSet: Array<IAnnotationMap>
  ): Promise<boolean> {
    return Promise.resolve(
      matchSet.some((annotationSet: IAnnotationMap) => {
        ...
        for (const kvp of Object.entries(annotationSet)) {
          if (
            ...
            (pod.metadata?.annotations[kvp[0]] == null ||
              pod.metadata.annotations[kvp[0]] !== kvp[1])
          )
            return false
        }
  ...
  isIgnored(pod: V1Pod): Promise<boolean> {
    ...
    if (pod.metadata == null || pod.metadata.annotations == null)
